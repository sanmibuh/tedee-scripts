...
│   ├── tedee.conf         # Your config (not in git)
│   └── tedee.conf.template # Template to copy
...
```bash
cp config/tedee.conf.template config/tedee.conf
```

Edit `config/tedee.conf` and fill in:
- `BRIDGE_IP` - Your Tedee Bridge IP address
- `TEDEE_TOKEN` - Your Tedee API token
- `DEVICE_ID` - Your Tedee lock device ID
